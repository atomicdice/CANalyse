...
def settings_menu():
    while True:
        ...
        print("")
        tc.cprint(
            "1) Communication channel : " + settings["comm_channel"], settings["color"]
        )
        tc.cprint("2) Color : " + settings["color"], settings["color"])
        tc.cprint("3) source filename : " + settings["source"], settings["color"])
        tc.cprint(
            "4) Primary attack filename : " + settings["attack"], settings["color"]
        )
        tc.cprint(
            "5) Secondary attack filename : " + settings["sec_attack"],
            settings["color"],
        )
        tc.cprint("6) Payload filename : " + settings["payload"], settings["color"])
        tc.cprint(
            "7) Communication Interface : " + settings["interface"], settings["color"]
        )
    ...
    while True:
        ...
        if z == 1:
            ...
            cn.write(
                df4,
                settings["filemode"],
                settings["payload"] + "_" + str(l[z - 2]) + ".log",
            )
...
def play_payload(cn, x):
    if x == "":
        playthread = threading.Thread(
            target=cn.canplay, args=(settings["payload"] + ".log",)
        )
        showthread = threading.Thread(
            target=cn.show_log,
            args=(
                settings["payload"] + ".log",
                "all",
            ),
        )
    else:
        playthread = threading.Thread(
            target=cn.canplay, args=(settings["payload"] + "_" + str(x) + ".log",)
        )
        showthread = threading.Thread(
            target=cn.show_log,
            args=(
                settings["payload"] + "_" + str(x) + ".log",
                str(x),
            ),
        )
    showthread.start()
...
def exec_message(bot, msg, cn):
    ...
    if text[0] == "record":
        ...
        options = [
            "'record (source / attack) (seconds)' to record the files ",
            "'play (no_of times) (filename) (can_id {optional})' to play the recorded/analysed files",
            "'analyse source attack' to analyse the files",
            "'get (source / attack / payload)' to download any file",
            "'list' to view contents of data folder",
            "'clean' to erase the contents of data folder.",
        ]
...
def telegram_play(bot, msg, filename, cn, can_id=None):
    ...
    if can_id != None:
        ...
        print(df)
        cn.write(
            df, settings["filemode"], settings["payload"] + "_" + str(can_id) + ".log"
        )
...
def telegram_analyse(bot, msg, source, attack, cn):
    ...
    bot.send_message(
        chat_id=chat_id, text="written payload at " + settings["payload"] + ".log"
    )
        ...
        if o == 1:
            ...
            while True:
                ...
                try:
                    ...
                    bot.send_message(
                        chat_id=chat_id, text="Invalid command type 'Menu' for manual"
                    )
            pass  # telegram
